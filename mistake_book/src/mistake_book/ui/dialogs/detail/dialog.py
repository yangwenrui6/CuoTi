...
class DetailDialog(QDialog):
    ...
    def __init__(self, controller, parent=None):
        ...
        Args:
            controller: DetailDialogController实例
            parent: 父窗口
        """
        super().__init__(parent)
        self.controller = controller
        self.question_data = controller.question_data
        ...
        logger.debug(f"DetailDialog初始化完成，题目ID: {self.question_data.get('id')}")
    ...
    def _init_ui(self):
        ...
        # 基本信息
        self._add_basic_info_section(content_layout)
        
        # 题目内容
        self._add_content_section(content_layout)
        
        # 答案部分
        self._add_answer_section(content_layout)
        
        # 复习数据
        self._add_review_data_section(content_layout)
        
        # 图片（如果有）
        self._add_image_section(content_layout)
    ...
    def _add_basic_info_section(self, layout):
        """添加基本信息"""
    ...
    def _add_content_section(self, layout):
        """添加题目内容 - 可编辑"""
    ...
    def _add_answer_section(self, layout):
        """添加答案部分 - 可编辑"""
        # 我的答案
    ...
    def _add_review_data_section(self, layout):
        """添加复习数据"""
    ...
    def _add_image_section(self, layout):
        """添加图片（如果有）"""
        ...
        # 获取完整路径
        if self.controller.question_service:
            full_path = self.controller.question_service.get_image_full_path(image_path_str)
        else:
            # 兼容旧代码，直接使用路径
    ...
    def _on_save_clicked(self):
        ...
        # 检查是否有修改
        if not self.controller.has_changes(current_data):
            QMessageBox.information(self, "提示", "没有修改需要保存")
            return
        
        # 调用控制器保存
        success, message = self.controller.save_changes(current_data)
    ...
    def _on_close_clicked(self):
        ...
        # 检查是否有未保存的修改
        if self.controller.has_changes(current_data):
